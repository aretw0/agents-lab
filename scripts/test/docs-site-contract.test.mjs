...
test("public 0.8 readiness map matches the canonical release readiness gate", () => {
	...
	assert.match(playbook, /release:final:gate -- --target X\.Y\.Z/);
	assert.match(playbook, /release:artifact:audit -- --target X\.Y\.Z/);
	assert.match(playbook, /schemaVersion=1/);
	assert.match(playbook, /markdown/);
	assert.match(playbook, /operatorDecisions/);
	assert.match(playbook, /requiredApprovalPrompts/);
...
test("publishing guide keeps release validation and commit format explicit", () => {
	...
	assert.match(guide, /pnpm run release:readiness:strict:json -- --target X\.X\.X/);
