...
export async function runPiDriver(options = {}) {
  ...
  const driverPayload = {
    ...
    ...(!options.operatorApproval && options.operator_approval ? { operator_approval: options.operator_approval } : {}),
    ...(!options.operatorApproval && !options.operator_approval && options.approve === true ? { operator_approval: structuredApproval() } : {}),
