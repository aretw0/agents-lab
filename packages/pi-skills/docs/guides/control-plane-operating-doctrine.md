...
Antes de promover nova pesquisa externa para foco protected, preencher intake target-agnostic local-safe (`docs/primitives/external-influence-intake-template.md`) com hipótese, valor/risco/esforço, consumidor pretendido, worker_suitable_slice, fan_in_contract, non_goals, canário e rollback. Validar com `node scripts/project/external-influence-intake-validate.mjs --file <intake> --json --pretty` antes de materializar task ou worker. Esse intake prepara a decisão; não autoriza execução externa automática, URL fetch, clone, install, release, publish, workflow dispatch ou `ant_colony`.
