...
describe("monitor-provider-patch command surface", () => {
	...
	it("does not resolve classifier model from provider recommendations alone", async () => {
	...
	it("reports configured provider-map status and divergent overrides without applying changes", async () => {
		mkdirSync(join(tmpDir, ".pi", "agents"), { recursive: true });
		mkdirSync(join(tmpDir, ".pi", "monitors"), { recursive: true });
		writeFileSync(
			join(tmpDir, ".pi", "settings.json"),
		...
		const [message, severity] = notify.mock.calls[0];
		expect(severity).toBe("info");
		expect(message).toContain("resolvedClassifierModel: github-copilot/claude-haiku-4.5 (provider-map)");
