...
describe("guardrails unattended continuation surface", () => {
  ...
  it("surfaces repeated context pressure as advisory stagnation signal", () => {
    const cwd = mkdtempSync(join(tmpdir(), "local-continuity-stagnation-"));
    try {
      execFileSync("git", ["init"], { cwd, stdio: "ignore" });
      execFileSync("git", ["config", "user.email", "test@example.com"], { cwd, stdio: "ignore" });
      execFileSync("git", ["config", "user.name", "Test User"], { cwd, stdio: "ignore" });
      ...
      writeFileSync(join(cwd, ".project", "handoff.json"), JSON.stringify({
        timestamp: new Date().toISOString(),
        current_tasks: ["TASK-SURFACE"],
        blockers: ["context-watch-compact-required"],
        context_watch_events: [
          { reason: "message_end", level: "checkpoint", percent: 70, action: "checkpoint-refresh" },
          { reason: "message_end", level: "compact", percent: 92, action: "compact-now" },
        ],
      }));
      writeFileSync(join(cwd, ".project", "tasks.json"), JSON.stringify({ tasks: [{
        id: "TASK-SURFACE",
