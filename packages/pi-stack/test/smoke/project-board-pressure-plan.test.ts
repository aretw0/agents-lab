...
describe("project-board pressure reduction", () => {
  ...
  it("archives completed history with explicit authorization", () => {
    ...
    try {
      ...
      const archivedTasks = JSON.parse(readFileSync(join(cwd, ".project", "archive", "completed-tasks.json"), "utf8"));
      const archivedVerification = JSON.parse(readFileSync(join(cwd, ".project", "archive", "verification-ledger.json"), "utf8"));
      ...
      expect(archivedTasks.tasks.map((task: any) => task.id)).toEqual(["TASK-DONE"]);
      expect(archivedVerification.verifications.map((row: any) => row.id)).toEqual(["VER-DONE"]);
      ...
      const archivedAgain = JSON.parse(readFileSync(join(cwd, ".project", "archive", "completed-tasks.json"), "utf8"));
      expect(archivedAgain.tasks).toHaveLength(1);
    } finally {
