...
describe("ops calibration decision packet", () => {
  ...
  it("builds start packet as ready-for-operator-decision when rehearsal and gates are green", () => {
    ...
    expect(packet.recommendationCode).toBe("delegation-rehearsal-start-ready-for-operator-decision");
    expect(packet.dispatchAllowed).toBe(false);
    expect(packet.authorization).toBe("none");
  ...
  it("applies telemetry-equivalent normalization to start packet when live fallback is eligible", async () => {
    ...
    try {
      ...
      expect(String(result.details.summary)).toContain("why=operator-canary-decision");
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
