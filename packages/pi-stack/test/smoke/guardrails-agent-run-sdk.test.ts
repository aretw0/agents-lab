...
import { buildAgentRunSdkCachePackPacket, buildAgentRunSdkInProcessPacket, buildAgentRunSdkReadOnlyBatchPacket, buildAgentRunSdkReadOnlyBatchTaskPacket } from "../../extensions/guardrails-core-exports";
...
describe("agent run SDK packet surfaces", () => {
  it("builds sdk in-process packet preview without dispatch", async () => {
    ...
    const batchDispatchMissingApproval = batchDispatchTool.execute("tc-sdk-batch-dispatch-missing-approval", {
      batch_id: "task-bud-1071-sdk-readonly-batch-dispatch-preview",
      shared_evidence: ["VERIF-TASK-BUD-1071-SDK-CACHE-PARALLEL-CONTRACT-20260512"],
      workers: [
        {
    ...
    expect(batchDispatchMissingApproval.details?.processStartAllowed).toBe(false);
    expect(batchDispatchMissingApproval.content?.[0]?.text).toContain("structured-operator-approval-missing");
    ...
    const surfaceResult = tool.execute("tc-sdk-preview", {
      ...
      rollback_plan_known: true,
      budget_decision: "ok",
      budget_evidence: "model-specific capacity available",
    ...
    const dispatchMissingApproval = dispatchTool.execute("tc-sdk-dispatch-missing-approval", {
      run_id: "task-bud-1068-sdk-dispatch-preview-canary",
    ...
    expect(dispatchMissingApproval.details?.processStartAllowed).toBe(false);
