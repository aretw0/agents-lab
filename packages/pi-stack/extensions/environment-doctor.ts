...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "environment_dev_pressure_status",
    label: "Environment Dev Pressure Status",
    description: "Read-only runtime pressure diagnostics for Pi sessions, configured package surfaces, and dev startup budget.",
    parameters: Type.Object({}),
    async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
      const { buildEnvironmentDevPressureReport } = await import("./environment-doctor-dev-pressure");
      const payload = buildEnvironmentDevPressureReport(ctx.cwd);
      return buildOperatorVisibleToolResponse({
  ...
  pi.registerCommand("doctor", {
    ...
    handler: async (args, ctx) => {
      ...
      if (mode === "pressure" || mode === "dev-pressure") {
        const { buildEnvironmentDevPressureReport } = await import("./environment-doctor-dev-pressure");
        const payload = buildEnvironmentDevPressureReport(ctx.cwd);
        const lines = [
          payload.summary,
          ...payload.signals.map((signal) => `  - [${signal.level}] ${signal.code}: ${signal.detail}`),
          `  sessions: count=${payload.sessions.count} totalMb=${payload.sessions.totalMb} largest=${payload.sessions.largest?.mb ?? 0} MB`,
          `  settings: ${payload.settings.map((row) => `${row.path} packages=${row.packageCount} suppressed=${row.suppressedSurfaceCount} extExcludes=${row.extensionExcludeCount}`).join("; ")}`,
        ];
        if (payload.configuredEntrypoints.length > 0) {
          lines.push("  heaviest configured entrypoints:");
          for (const row of payload.configuredEntrypoints.slice(0, 5)) {
            lines.push(`    - ${row.package}:${row.entry}: files=${row.files} kb=${row.kb} (${row.settings})`);
          }
        }
        ctx.ui.notify(lines.join("\n"), payload.recommendation === "continue" ? "info" : "warning");
        return;
      }
