...
import { ensureMonitorIssueWriteTemplateSchema } from "./monitor-provider-issue-template";
import { buildStatusReport, buildTemplateSnippet, planSessionStartOutput } from "./monitor-provider-output";

import { parseCommandInput, detectBooleanSetting, detectDefaultProvider, resolveClassifierModel, detectClassifierThinking, detectHedgeWhen, detectHedgeIncludeProjectContext, detectFragilityWhen, parseModelRef, ensureOverrides, syncOverrides, repairLegacyTemplateOverrides, repairMissingSystemPromptOverrides, readOverrideModels, checkModelAvailability } from "./monitor-provider-core";
export { ensureMonitorIssueWriteTemplateSchema } from "./monitor-provider-issue-template";
export { buildStatusReport, buildTemplateSnippet, planSessionStartOutput } from "./monitor-provider-output";
			...
			if (cmd === "help") {
				ctx.ui.notify(
					[
						...
						"  apply [provider/model] [model]  Sincroniza os 5 overrides para um modelo explícito",
				...
				if (!model) {
					ctx.ui.notify(
						[
							...
							"Defina piStack.monitorProviderPatch.classifierModel ou classifierModelByProvider,",
