...
export function registerGuardrailsStructuredInterviewSurface(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      worker_readiness_requested: Type.Optional(Type.Boolean({ description: "When true, route immediately to read-only runtime/subagent/provider readiness checks without preparing or dispatching a worker. The intake also infers this route from worker/subagent readiness intent text." })),
      brainstorm_requested: Type.Optional(Type.Boolean({ description: "When true, route to report-only brainstorm/seed preview. The intake also infers this route from next-slice/no-eligible/brainstorm intent text." })),
      no_eligible_local_safe_tasks: Type.Optional(Type.Boolean({ description: "When true, route to report-only brainstorm/seed preview instead of asking broad follow-up questions." })),
    ...
    execute(_toolCallId, params) {
      ...
      const packet = buildOperatorIntentIntakePacket({
        ...
        runtimeHealthRequested: p.runtime_health_requested === true,
        workerReadinessRequested: p.worker_readiness_requested === true,
