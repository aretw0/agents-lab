...
- no JSON, `operatorDecisions[*].boardReleaseDispositionPacket` aparece quando o board tem candidatos com evidência de release; ele lista `dispositionRows[*].recommendedAction` sem editar `.project/tasks.json` nem autorizar automação;
