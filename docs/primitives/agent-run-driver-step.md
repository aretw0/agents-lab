...
`agent-run:driver-canary` runs a bounded local `node --version` canary through the same driver step and writes `.artifacts/agent-run-driver/latest.json` for release readiness evidence.
