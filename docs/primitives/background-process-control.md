...
- `background_process_readiness_packet` — packet unificado (plan + readiness + rehearsal) com `nextAction` explícito (`ready-window|needs-evidence|blocked`) e `unlockChecklist` curto (top blockers + próxima ação).
