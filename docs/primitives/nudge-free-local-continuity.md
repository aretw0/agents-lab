...
| `turn_boundary_decision_packet` | Explicar parada/continuação no boundary do turno (`continue|checkpoint|pause|ask-human`) com `humanActionRequired`, `localSafeMayContinue`, `nextAutoStep` e preview direcional (`similar-lane` vs `next-high-value`). | Report-only/read-only; sem dispatch e com motivo canônico auditável. |
