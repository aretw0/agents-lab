...
| `failure-contract` | pending canary | Preview prepared: `task-bud-1076-codex-spark-failure-contract-canary` | Serial exact-confirmed canary should prove the model fails closed with explicit `FAIL; missing-evidence` output instead of looping or inventing evidence. |
...
## Next canary to unlock remaining arena envelope

Prepared preview only; no dispatch occurred:

- run id: `task-bud-1076-codex-spark-failure-contract-canary`
- provider/model: `openai-codex/gpt-5.3-codex-spark`
- declared file: `docs/research/worker-provider-model-arena-2026-05.md`
- tools: `read`
- timeout: `45000ms`
- expected final output: one concise line starting `FAIL; missing-evidence`, then stop
- human confirmation phrase from packet: `execute o sdk worker task-bud-1076-codex-spark-failure-contract-canary`

This canary should be run serially only after explicit operator confirmation and budget evidence.
