...
| `readonly-source-backed-evidence-synthesis` | liberated | `task-bud-1076-codex-spark-readonly-source-backed-prior-art-synthesis-canary` completed with ADOPT/ADAPT/REJECT/PARENT-CHECK sections, outputBytes=2134, no touched files, and parent outcome contract=pass | Three declared files including one source-backed evidence packet, `read`/`grep`, fixed synthesis output, and missing-source claims routed to PARENT-CHECK; no autonomous web research, command execution, or mutation. |

## Explicit non-promotions

- Autonomous web/source research by a worker is **not** liberated. The source-backed run validated synthesis over a parent-curated evidence packet only. A future worker-research envelope requires a curated research tool, domain/source allowlist, citations/permalinks, timeout/budget caps, and fail-closed behavior when sources are missing.
- True prior-art synthesis from a declared evidence packet is liberated only as `readonly-source-backed-evidence-synthesis`: the worker may synthesize cited/cached evidence already supplied by the parent. Model weights are not evidence.
