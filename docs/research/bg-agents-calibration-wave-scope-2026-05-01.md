...
- [ ] rodar as duas tools novas após reload e registrar score real da sessão
- [ ] manter decisão `report-only` se qualquer score ficar abaixo de 70
- [ ] só discutir promoção operacional quando ambos os scores ficarem >= 80
- [ ] registrar checkpoint com decisão explícita (avançar/pausar)

## Runbook curto pós-reload (leitura live)

1. executar `/reload` para ativar as novas surfaces;
2. invocar `background_process_readiness_score`;
3. invocar `agents_as_tools_calibration_score`;
4. invocar `ops_calibration_decision_packet` com `live_reload_completed=true`;
5. registrar checkpoint com scores, recommendationCode e decisão go/no-go para rehearsal local.
