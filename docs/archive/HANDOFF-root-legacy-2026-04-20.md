...
> **Modo delta (regra vigente):** o estado canônico vive em `.project/*` (tasks/requirements/decisions/handoff).
> Este arquivo deve registrar só **delta operacional curto** (retomada humana), sem duplicar o board.
> Regra de foco de execução: **WIP=1 por sessão**.
> Referência operacional: `docs/guides/project-canonical-pipeline.md`.
...
### 2.5 Diagnóstico rápido (2026-04-20) — warning dos monitores `[... classify failed ... Instructions are required]`

Status: **reproduzido + causa localizada no runtime do monitor engine**.

Evidência técnica mínima (sem session triage pesado):

- `monitors_compact_status` reportou `classifyFail=3`.
- Em `@davidorex/pi-behavior-monitors/dist/index.js` (`classifyViaAgent`):
  - classificação usa `complete(..., { tools: [VERDICT_TOOL], toolChoice: { type: "tool" ... } })`;
  - se não houver `toolCall`, retorna erro `No tool call in response ...`;
  - com `openai-codex`, resposta observada inclui `error: {"detail":"Instructions are required"}`.
- Portanto, o warning **não é ruído do hedge em si** (patterns/instructions estão carregados), e sim falha de compatibilidade no caminho de classificação com provider atual.

Mitigação operacional aplicada nesta sessão:

- `monitors-control off` executado para evitar flood de warning/token enquanto calibramos.

Próximo passo recomendado (curto):

1. manter monitores pausados por default durante calibração OpenAI-only;
2. abrir patch upstream (ou fork local) para compatibilizar `classifyViaAgent` com providers que exigem `instructions` explícitas;
3. só então reativar monitores gradualmente (`hedge` -> `fragility` -> restantes).
...
## WIP de modularização (2026-04-20) — parada segura para próxima sessão

Status atual: **split incremental avançou + validação verde**.

Entregas desta retomada:

- módulo já criado e integrado:
  - `packages/pi-stack/extensions/colony-pilot-model-readiness.ts`
  - extrações: `parseProviderModelRef`, `resolveModelAuthStatus`, `resolveColonyModelReadiness`, `formatModelReadiness`, tipos relacionados.
- novo módulo criado e integrado:
  - `packages/pi-stack/extensions/colony-pilot-preflight.ts`
  - extrações: `ColonyPilotPreflightConfig`, `ColonyPilotPreflightResult`, `resolveColonyPilotPreflightConfig`, `executableProbe`, `runColonyPilotPreflight`, `formatPreflightResult`.
- `packages/pi-stack/extensions/colony-pilot.ts` atualizado para importar/reexportar símbolos extraídos.

Validação executada:

- `"/mnt/c/Users/aretw/scoop/apps/nodejs/current/node.exe" ./node_modules/vitest/vitest.mjs run packages/pi-stack/test/smoke/colony-pilot-parsers.test.ts` → **PASS (65/65)**

Arquivos alterados nesta fase:

- `packages/pi-stack/extensions/colony-pilot.ts`
- `packages/pi-stack/extensions/colony-pilot-model-readiness.ts` (novo)
- `packages/pi-stack/extensions/colony-pilot-preflight.ts` (novo)
- `HANDOFF.md`
- `ROADMAP.md`

Próximo passo imediato na retomada:

1. extrair o próximo bloco grande (`modelPolicy` ou `budgetPolicy`) para módulo dedicado;
2. rerodar smoke alvo (`colony-pilot-parsers`) e, se possível, um smoke adicional de integração de colony-pilot.

## Protocolo temporário de contexto (até existir automação de compactação)

Objetivo: manter continuidade sem estourar janela de contexto.

- **60–70%**: executar apenas tarefas lineares e curtas (1 bloco por vez), sem abrir novas frentes.
- **70–80%**: congelar escopo, finalizar validação mínima e atualizar handoff curto.
- **>80%**: parar execução de desenvolvimento e compactar sessão.
- **Formato mínimo de handoff por ciclo**:
  1) mudanças feitas,
  2) validação executada,
  3) estado atual dos arquivos,
  4) próximo único passo.
- **Enquanto calibramos monitores/custos**: manter monitores pausados por padrão nas sessões de refactor longo.

## Snapshot final pré-compactação (2026-04-20)

- validação mais recente: `colony-pilot-parsers` **PASS (65/65)**.
- working tree atual:
  - `M HANDOFF.md`
  - `M ROADMAP.md`
  - `M packages/pi-stack/extensions/colony-pilot.ts`
  - `?? packages/pi-stack/extensions/colony-pilot-model-readiness.ts`
  - `?? packages/pi-stack/extensions/colony-pilot-preflight.ts`
- próximo passo único após compactação:
  1. extrair **`modelPolicy`** para módulo dedicado,
  2. rerodar `colony-pilot-parsers`.
