...
1. `pnpm run project:verification:check`
...
- [ ] Se houver bloqueio hard-intent: registrar `ask-operator` com 1 pergunta objetiva + 1 ação segura proposta.
...
- **Se gate determinístico bloquear:** parar em `ask-operator`, fazer 1 pergunta objetiva, oferecer 1 próximo passo seguro e não sugerir dispatch/auto-continuação implícita.
