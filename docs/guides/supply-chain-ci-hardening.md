...
- `release:package:smoke:json` emits `mode=release-package-smoke-report`, `schemaVersion=1`, `decision=pass|block`, structured `packageBlockers`/`packageWarnings` and report-only `automationPermissions`; consumers must not treat this report as publish or workflow authorization.
