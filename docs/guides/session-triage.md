...
Quando a intenção do operador ainda está livre ou ambígua, rode primeiro `operator_intent_intake_packet`. Ele decide, sem mutação ou dispatch, se o próximo passo é entrevista curta, brainstorm de lane, single-slice local ou readiness de runtime/worker. Se o operador pedir só saúde do runtime antes de trabalhar, inclusive mencionando watchdog ou colando `/watchdog:*`, passar o texto como intenção basta: o packet infere `runtime_health_requested=true`. `needs-evidence` nesse caminho significa coletar evidência mínima, não promover automaticamente para `stop-and-investigate`. Se perguntar se workers/subagentes estão prontos ou seguros, a intake infere `worker_readiness_requested=true` e checa readiness sem preparar nem despachar worker; orçamento só entra ao preparar pacote de worker ou dispatch. Se safe-mode do watchdog estiver ativo, a rota continua pequena/read-only e bloqueia escalation para worker dispatch, pi-lens, web gateway, remote/offload ou publish até estabilidade ou decisão explícita nova. Se pedir a próxima fatia local-safe, disser que não há tarefas elegíveis ou pedir seed/brainstorm do board, a intake infere a rota `lane_brainstorm_packet` + `lane_brainstorm_seed_preview` sem materializar task. Quando retornar `reportOnlyRouteAuthorized=true`, execute as ferramentas reais indicadas pela rota read-only recomendada, não sintetize o packet por inferência, e não peça confirmação textual.
