...
- hard intent de continuidade (escopo long-run): em `compact`, quando `autoResumeAfterCompact=yes` e `compact-checkpoint-persist: recommended=yes` (ver `/context-watch status`), persistir handoff antes de encerrar/parar mesmo quando houver cooldown de announce/checkpoint.
