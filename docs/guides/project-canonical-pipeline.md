...
- `/lane-queue evidence` também aceita override de milestone com o mesmo contrato e inclui `boardReadiness` scoped + `boardHint` quando não há elegível no recorte informado, além de `scopeParity` (expected/boardAuto/loopReady) para diagnóstico rápido de consistência de escopo.
