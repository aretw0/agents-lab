...
A primitiva `resolveHumanConfirmationAuditPlan` formaliza essa distinção em código: `uiConfirmationObserved=true` sem evidência monitor-visível retorna `decision=audit-gap`, `dispatchAllowed=false`, `canOverrideMonitorBlock=false` e `authorization=none`. Mesmo evidência trusted/exact-match retorna apenas `decision=auditable`; ela não executa nem aprova dispatch. A camada core também define `resolveHumanConfirmationEvidenceMatch` e `consumeTrustedHumanConfirmationEvidence` para TTL, match exato de `toolName`/path/scope/payload e consumo single-use; isso prepara a ponte runtime sem tornar texto livre ou confirmação stale em autorização. O envelope `buildTrustedHumanConfirmationAuditEnvelope` usa `customType=human-confirmation-evidence`, `display=false`, resumo bounded e `dispatchAllowed=false`/`canOverrideMonitorBlock=false`, para que uma integração futura possa gravar audit/custom-message sem criar permissão operacional por acidente.
