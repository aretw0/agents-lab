...
Para inspeção de dirty state, prefira snapshot sem arquivo temporário (`npm run git:dirty:snapshot`, command `/git-dirty`, ou tool read-only `git_dirty_snapshot`) em vez de `git diff --name-only > arquivo` seguido de `rm`.
